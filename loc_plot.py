...
#         self.selected_columns = selected_columns
#         self.point_list = point_list # List widget connected to the map widget
#         self.mode = "Latitude/Longitude" # Default
#         self.selector = None
...
#     def plot_localization(self, mode):
#         self.mode = mode # Update the mode if the user clicks the radio button
#         self.ax.clear()
...
#         # Handle invalid coordinates (click outside axes)
#         if None in (x1, y1, x2, y2):
#             return
...
#             if len(wells) > 0:
#                 # Collect currently listed wells
#                 current_wells = {self.point_list.item(i).text() for i in range(self.point_list.count())}
#                 # Add only new ones
#                 for well in wells:
#                     if str(well) not in current_wells:
